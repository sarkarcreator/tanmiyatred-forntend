    ...
    <div className="min-h-screen bg-[#0A0A09] flex items-center justify-center p-4">
      <div className="w-full max-w-md bg-[#171715] border border-[#25221E] p-8 sm:p-10 space-y-6">
        <div className="text-center space-y-4">
          ...
          <div className="pt-2"><span className="text-xs uppercase tracking-[0.25em] text-[#B79A62] font-semibold">Executive Management & CRM Portal</span><p className="text-xs text-[#8C867E] mt-1">Authorized Personnel Only • RERA, DLD & Corporate Compliance</p></div>
        </div>
        {authError && <div className="p-3 bg-red-950/50 border border-red-800 text-red-200 text-xs">{authError}</div>}
        <form onSubmit={handleLogin} className="space-y-4">
          <div><label className="block text-xs uppercase tracking-wider text-[#C8C0B3] mb-1">Executive Email</label><input type="email" required value={authEmail} onChange={(e) => setAuthEmail(e.target.value)} className="w-full bg-[#0A0A09] border border-[#2D2A26] px-3.5 py-2.5 text-sm text-[#F5F2EB] focus:outline-none focus:border-[#B79A62]" /></div>
          <div><label className="block text-xs uppercase tracking-wider text-[#C8C0B3] mb-1">Security Passcode</label><input type="password" required value={authPassword} onChange={(e) => setAuthPassword(e.target.value)} className="w-full bg-[#0A0A09] border border-[#2D2A26] px-3.5 py-2.5 text-sm text-[#F5F2EB] focus:outline-none focus:border-[#B79A62]" /></div>
          <button type="submit" className="w-full py-3.5 bg-[#B79A62] text-[#0A0A09] text-xs font-semibold uppercase tracking-[0.2em] hover:bg-[#D8BE8A] transition-colors cursor-pointer">AUTHENTICATE SESSION</button>
        </form>
      </div>
    </div>
  );
  ...
  return (
    <div className="min-h-screen bg-[#0A0A09] text-[#F5F2EB] flex flex-col">
      <header className="bg-[#171715] border-b border-[#25221E] px-6 py-4 flex items-center justify-between sticky top-0 z-40">
        <div className="flex items-center gap-6"><TanmiyatLogo variant="gold" size="sm" showDescriptor={false} /><span className="hidden sm:inline-block text-xs uppercase tracking-widest text-[#8C867E] border-l border-[#2B2925] pl-6">Luxury Real Estate OS & CRM</span></div>
        <div className="flex items-center gap-4"><div className="flex items-center gap-2 px-3 py-1 bg-[#0A0A09] border border-[#2D2A26] text-xs"><Shield className="w-3.5 h-3.5 text-[#B79A62]" /><span className="text-[#F5F2EB] font-medium">SUPER_ADMIN</span></div><a href="/" target="_blank" className="text-xs text-[#C8C0B3] hover:text-[#B79A62] flex items-center gap-1 transition-colors"><span>Live Site</span><ExternalLink className="w-3.5 h-3.5" /></a><button onClick={handleLogout} className="text-[#8C867E] hover:text-red-400 p-1 transition-colors cursor-pointer" title="Sign Out"><LogOut className="w-4 h-4" /></button></div>
      </header>
      <div className="flex-grow flex flex-col md:flex-row">
        <aside className="w-full md:w-64 bg-[#121210] border-r border-[#25221E] p-4 flex flex-col justify-between shrink-0">
          <div className="space-y-6">{navGroups.map((group, gIdx) => <div key={gIdx} className="space-y-1"><span className="px-3 text-[10px] uppercase tracking-[0.2em] text-[#7A756D] font-bold block mb-2">{group.group}</span>{group.items.map((tab) => { const Icon = tab.icon; const isActive = activeTab === tab.id; return <button key={tab.id} onClick={() => setActiveTab(tab.id)} className={`w-full flex items-center justify-between px-3.5 py-2.5 text-xs uppercase tracking-wider font-medium transition-all cursor-pointer ${isActive ? 'bg-[#B79A62] text-[#0A0A09] font-semibold' : 'text-[#C8C0B3] hover:bg-[#1A1816] hover:text-[#F5F2EB]'}`}><div className="flex items-center gap-3"><Icon className="w-4 h-4 shrink-0" /><span className="truncate">{tab.label}</span></div>{tab.count !== undefined && tab.count > 0 && <span className={`text-[10px] px-1.5 py-0.2 rounded shrink-0 ${isActive ? 'bg-[#0A0A09] text-[#B79A62] font-bold' : 'bg-[#25221E] text-[#C8C0B3]'}`}>{tab.count}</span>}</button>; })}</div>)}</div>
          <div className="p-4 bg-[#0A0A09] border border-[#22201C] text-[11px] text-[#7A756D] space-y-1 mt-6"><p className="font-semibold text-[#B79A62]">Tanmiyat CRM Core</p><p>RERA Form A & B: Verified</p><p>DLD Escrow: Active</p></div>
        </aside>
        <main className="flex-grow p-6 sm:p-10 overflow-x-hidden max-w-7xl">
          {activeTab === 'OVERVIEW' && (
            <div className="space-y-8">
              <div><h1 className="font-editorial text-3xl text-[#F5F2EB]">Executive Overview</h1><p className="text-xs text-[#8C867E]">Real-time portfolio metrics, CRM leads velocity, marketplace listings, and escrow pipeline.</p></div>
          ...
          {activeTab === 'AGENTS' && <AgentsManager agents={agents} onChange={setAgents} />}
          {activeTab === 'VIEWINGS' && <ViewingsManager viewings={viewings} properties={properties} agents={agents} onChange={setViewings} />}
          {activeTab === 'OFFERS' && <OffersManager offers={offers} properties={properties} agents={agents} onChange={setOffers} />}
          {activeTab === 'DEALS' && <DealsManager deals={deals} properties={properties} agents={agents} onChange={setDeals} />}
          {activeTab === 'COMMISSIONS' && <CommissionsManager commissions={commissions} deals={deals} agents={agents} onChange={setCommissions} />}
          {activeTab === 'DEVELOPMENTS' && <div className="space-y-4"><h2 className="font-editorial text-2xl">Master Projects</h2><p className="text-xs text-[#8C867E]">Project management remains available through the existing development data layer.</p></div>}
          {activeTab === 'UNITS' && <div className="space-y-4"><h2 className="font-editorial text-2xl">Inventory Units</h2><p className="text-xs text-[#8C867E]">{allUnits.length} inventory units currently indexed.</p></div>}
          {activeTab === 'TIMELINE' && <div className="space-y-4"><h2 className="font-editorial text-2xl">Heritage Timeline</h2><p className="text-xs text-[#8C867E]">{timeline.length} timeline records available.</p></div>}
          {activeTab === 'NEWS' && <div className="space-y-4"><h2 className="font-editorial text-2xl">News & Insights</h2><p className="text-xs text-[#8C867E]">{news.length} news records available.</p></div>}
          {activeTab === 'USERS' && <div className="space-y-4"><h2 className="font-editorial text-2xl">Roles & Access</h2><p className="text-xs text-[#8C867E]">Executive access is protected by the configured authentication layer.</p></div>}
        </main>
      </div>
    </div>
  );
