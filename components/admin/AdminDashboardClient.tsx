...
export type AdminTab =
  | 'OVERVIEW' | 'LEADS_CRM' | 'PROPERTIES' | 'AGENTS' | 'VIEWINGS' | 'OFFERS'
  | 'DEALS' | 'COMMISSIONS' | 'DEVELOPMENTS' | 'UNITS' | 'TIMELINE' | 'NEWS' | 'USERS';

type JsonResponse = { success?: boolean; data?: unknown; error?: string };

const readJson = async (res: Response): Promise<JsonResponse> => {
...
export const AdminDashboardClient: React.FC<AdminDashboardClientProps> = ({
  ...
  const allUnits = projects.flatMap((project) =>
    (project.units || []).map((unit) => ({ ...unit, projectName: project.title }))
  );
  ...
  const loadAdminData = async () => {
    const endpoints = [
      ...
      ['news', '/api/news'],
      ['properties', '/api/properties?limit=24'],
      ['leads', '/api/leads'],
      ['agents', '/api/agents'],
    ...
    const results = await Promise.allSettled(
      endpoints.map(async ([, url]) => {
        ...
        if (!res.ok || !json.success) {
          throw new Error(json.error || `Failed to load CRM data (HTTP ${res.status}).`);
        }
        return json.data;
      })
    );
    ...
    results.forEach((result, index) => {
      data[endpoints[index][0]] = result.status === 'fulfilled' ? result.value : [];
    });
  ...
  if (authChecking) {
    return (
      <div className="min-h-screen bg-[#0A0A09] flex items-center justify-center p-6 text-[#B79A62] text-xs uppercase tracking-[0.2em]">
  ...
  const navGroups = [
    {
      ...
      items: [
        ...
        { id: 'VIEWINGS' as AdminTab, label: 'Private Viewings', icon: Calendar, count: viewings.filter((viewing) => viewing.status === 'CONFIRMED' || viewing.status === 'REQUESTED').length },
        { id: 'OFFERS' as AdminTab, label: 'Offers & Negotiations', icon: FileCheck, count: offers.filter((offer) => offer.status === 'SUBMITTED').length },
        ...
        <main className="flex-grow p-6 sm:p-10 overflow-x-hidden max-w-7xl">
          {activeTab === 'OVERVIEW' && (
            <div className="space-y-8">
              ...
              <DashboardStats
                ...
                activeUnits={allUnits.filter((unit) => unit.status === 'AVAILABLE').length}
                totalUnits={allUnits.length}
                activeLeads={leads.length || inquiries.length}
                activeListings={properties.length}
                upcomingViewings={viewings.filter((viewing) => viewing.status === 'REQUESTED' || viewing.status === 'CONFIRMED').length}
                activeOffers={offers.filter((offer) => offer.status === 'SUBMITTED').length}
                activeDeals={deals.filter((deal) => deal.status !== 'COMPLETED' && deal.status !== 'CANCELLED').length}
                totalCommissions={commissions.reduce((sum, commission) => sum + (Number(commission.amount) || 0), 0)}
              />
            </div>
          )}

          {activeTab === 'LEADS_CRM' && <LeadsKanban initialLeads={leads} agents={agents} />}
          {activeTab === 'PROPERTIES' && <PropertiesManager properties={properties} agents={agents} onChange={setProperties} />}
          ...
          {activeTab === 'DEVELOPMENTS' && (
            <div className="space-y-4">
              <h2 className="font-editorial text-2xl">Master Projects</h2>
              <p className="text-xs text-[#8C867E]">Project management remains available through the existing development data layer.</p>
            </div>
          )}
          {activeTab === 'UNITS' && (
            <div className="space-y-4">
              <h2 className="font-editorial text-2xl">Inventory Units</h2>
              <p className="text-xs text-[#8C867E]">{allUnits.length} inventory units currently indexed.</p>
            </div>
          )}
          {activeTab === 'TIMELINE' && (
            <div className="space-y-4">
              <h2 className="font-editorial text-2xl">Heritage Timeline</h2>
              <p className="text-xs text-[#8C867E]">{timeline.length} timeline records available.</p>
            </div>
          )}
          {activeTab === 'NEWS' && (
            <div className="space-y-4">
              <h2 className="font-editorial text-2xl">News & Insights</h2>
              <p className="text-xs text-[#8C867E]">{news.length} news records available.</p>
            </div>
          )}
          {activeTab === 'USERS' && (
            <div className="space-y-4">
              <h2 className="font-editorial text-2xl">Roles & Access</h2>
              <p className="text-xs text-[#8C867E]">Executive access is protected by the configured authentication layer.</p>
            </div>
          )}
        </main>
      </div>
