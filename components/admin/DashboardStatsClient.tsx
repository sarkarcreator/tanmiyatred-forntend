...
import { Building2, Inbox, Layers, RotateCw, AlertCircle, Handshake } from 'lucide-react';

export type DashboardStatType = 'PROJECTS' | 'INQUIRIES' | 'UNITS' | 'HERITAGE' | 'REVENUE' | 'LEADS' | 'PROPERTIES' | 'DEALS' | 'VIEWINGS' | 'COMMISSIONS';
...
export const DashboardStats: React.FC<DashboardStatsProps> = ({
  ...
  const currentStats: DashboardStatsData = hasPropData
    ...
    : initialStats || fetchedStats || { totalProjects: 0, totalInquiries: 0, activeUnits: 0, newInquiries: 0, totalUnits: 0 };
  ...
  if (loading && !hasData) {
    return <section id="dashboard-stats-loading" aria-label="Loading dashboard statistics" className={`w-full space-y-3 ${className}`}>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5 sm:gap-6">{[1, 2, 3, 4].map((i) => <div key={i} className="bg-[#171715] border border-[#25221E] p-6 space-y-4 animate-pulse"><div className="h-3 w-24 bg-[#25221E] rounded-sm" /><div className="h-10 w-20 bg-[#25221E] rounded-sm" /><div className="h-3 w-36 bg-[#25221E] rounded-sm" /></div>)}</div>
    </section>;
  }

  if (error && !hasData) {
    return <div id="dashboard-stats-error" className={`bg-[#171715] border border-red-900/50 p-6 flex items-center justify-between gap-4 text-xs ${className}`}><div className="flex items-center gap-3 text-red-300"><AlertCircle className="w-5 h-5 shrink-0 text-red-400" /><span>Unable to retrieve executive metrics: {error}</span></div><button onClick={handleRefresh} className="px-4 py-2 bg-[#25221E] hover:bg-[#322E29] text-[#F5F2EB] uppercase tracking-wider font-semibold transition-colors">Retry Connection</button></div>;
  }
  ...
  return <section id="executive-dashboard-stats-grid" className={`w-full space-y-4 ${className}`}>
    ...
    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5 sm:gap-6">{cards.map((card) => { const Icon = card.icon; const isClickable = Boolean(onStatClick); return <div key={card.id} id={card.id} onClick={() => onStatClick?.(card.statType)} role={isClickable ? 'button' : 'region'} tabIndex={isClickable ? 0 : undefined} className={`group relative bg-[#171715] border border-[#25221E] hover:border-[#3D3830] p-6 transition-all duration-300 flex flex-col justify-between ${isClickable ? 'cursor-pointer hover:bg-[#1C1A17] focus:outline-none focus:ring-1 focus:ring-[#B79A62]' : ''}`}><div className="flex items-start justify-between gap-4"><div className="space-y-1.5"><span className="text-[11px] uppercase tracking-[0.2em] text-[#8C867E]">{card.label}</span><div className="text-3xl font-semibold text-[#F5F2EB]">{card.value}</div></div><Icon className="w-8 h-8 text-[#B79A62]" /></div><div className="mt-5 space-y-3"><p className="text-xs text-[#8C867E] leading-relaxed">{card.subtitle}</p><span className="inline-flex px-2 py-1 border border-[#3D3830] text-[10px] uppercase tracking-wider text-[#B79A62]">{card.badge}</span></div></div>; })}</div>
  </section>;
};
