    ...
    <footer className="bg-[#0A0A09] border-t border-[#22201C] pt-16 pb-12 text-[#C8C0B3]">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-10 pb-16 border-b border-[#22201C]">
          ...
          <div className="space-y-4">
            ...
            <ul className="space-y-2.5 text-xs">
              <li>
                <Link href="/developments/living-legends" className="hover:text-[#B79A62] transition-colors flex items-center justify-between">
                  <span>Living Legends</span>
                  <ArrowUpRight className="w-3 h-3 text-[#B79A62]" />
                </Link>
              </li>
              <li>
                <Link href="/developments/the-court-tower" className="hover:text-[#B79A62] transition-colors flex items-center justify-between">
                  <span>The Court Tower</span>
                  <ArrowUpRight className="w-3 h-3 text-[#B79A62]" />
                </Link>
              </li>
              <li>
                <Link href="/developments/the-exchange-tower" className="hover:text-[#B79A62] transition-colors flex items-center justify-between">
                  <span>The Exchange Tower</span>
                  <ArrowUpRight className="w-3 h-3 text-[#B79A62]" />
                </Link>
              </li>
              <li>
                <Link href="/developments/horizon-residences" className="hover:text-[#B79A62] transition-colors flex items-center justify-between">
                  <span>Horizon Residences</span>
                  <ArrowUpRight className="w-3 h-3 text-[#B79A62]" />
                </Link>
              </li>
              <li>
                <Link href="/developments" className="text-[#B79A62] hover:underline pt-1 inline-block">
        ...
        <div className="pt-8 flex flex-col md:flex-row items-center justify-between gap-4 text-xs text-[#736E66]">
          <p className="text-center md:text-left">{t.footer.legal}</p>
          <div className="flex items-center space-x-6 rtl:space-x-reverse shrink-0">
            <Link href="/privacy" className="hover:text-[#C8C0B3] transition-colors">{t.footer.privacy}</Link>
            <span>•</span>
            <Link href="/terms" className="hover:text-[#C8C0B3] transition-colors">{t.footer.terms}</Link>
            <span>•</span>
            <Link href="/cookies" className="hover:text-[#C8C0B3] transition-colors">{t.footer.cookies}</Link>
          </div>
        </div>
      </div>
    </footer>
